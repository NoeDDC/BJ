...
Ajouter une bonne journée ouvre l'énigme du jour : la réponse se tape dans un champ. La bonne réponse déclenche des confettis ; une mauvaise le dit et laisse réessayer autant de fois qu'on veut, sans jamais dévoiler la solution. La comparaison ignore majuscules, accents, ligatures (`œ`/`æ`), ponctuation et article de tête — inutile donc de lister « le vent » **et** « un vent » dans `alt` : la même entrée les couvre déjà. `alt` ne sert qu'aux pluriels et aux vrais synonymes, énigme par énigme. Après deux tentatives manquées, un bouton « Un indice ? » propose le texte du champ `hint` : ces indices sont faits pour être personnels (souvenirs, private jokes), pas des définitions. On peut fermer le popup et y revenir par le bouton 🧩 en haut à gauche, qui n'apparaît qu'une fois la journée validée et reste jusqu'à minuit. L'énigme est déduite de la date, donc identique pour Zoé et Noé, et la liste (`RIDDLES` dans `public/js/app.js`, une ligne par énigme) défile en entier avant qu'une énigme ne revienne — 73 énigmes, soit dix semaines et demie sans répétition. Les `hint` sont volontairement personnalisables : ceux livrés sont chaleureux mais génériques, et remplacer l'un d'eux par un souvenir ou une private joke (voir celui du peigne) vaut toujours mieux qu'une définition. La journée est comptée dans tous les cas, l'énigme ne conditionne rien.
